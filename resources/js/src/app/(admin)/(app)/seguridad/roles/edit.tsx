...
export default function RoleEdit() {
    ...
    const fetchData = async () => {
      try {
        const [permsRes, roleRes] = await Promise.all([
          axios.get('/api/admin/permisos'),
          axios.get(`/api/admin/roles/${id}`)
        ]);
        
        setPermissionsGrouped(permsRes.data);
        const keys = Object.keys(permsRes.data);
        if (keys.length > 0) {
        ...
        alert('Rol no encontrado o sin permisos para editarlo.');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(`/api/admin/roles/${id}`, formData);
      navigate('/admin/seguridad/roles');
    } catch (error: any) {
  ...
  if (isLoading) {
    return <div className="p-10 text-center text-gray-500">Cargando datos del rol...</div>;
  }
  ...
  return (
    <>
      <PageMeta title="Editar Rol" />
      <main className="p-6 bg-[#f8fafc] pb-24">
        
        {/* Module Header */}
        <ModuleHeader
          title="Editar Rol"
          description="Modifica los detalles del rol y sus permisos correspondientes"
