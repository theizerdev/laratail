<div>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-900">Editar Cotización {{ $order->numero }}</h2>
            <p class="mt-1 text-sm text-gray-500">Modifica la información de la cotización.</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 space-y-6">
                    ...
                    @if($customerSearch && count($customers) > 0)
                        <div class="absolute z-20 mt-1 w-full rounded-xl border border-gray-200 bg-white shadow-lg max-h-40 overflow-y-auto">
                            @foreach($customers as $c)
                                <button wire:click="$set('customer_id', {{ $c->id }}); $set('customerSearch', '')" class="flex items-center gap-3 w-full px-4 py-2.5 text-left text-sm hover:bg-amber-50 transition-colors">
                                    <div class="w-8 h-8 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center text-xs font-bold">{{ strtoupper(substr($c->nombre, 0, 1)) }}</div>
                                    <div>
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm">
                ...
                <flux:textarea wire:model="notas_cliente" label="Notas del cliente" rows="2" placeholder="Requerimientos del cliente..." />
                <div class="mt-3">
                    <flux:textarea wire:model="notas_internas" label="Notas internas" rows="2" placeholder="Observaciones internas..." />
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            {{-- Resumen --}}
            <div class="rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 p-5 shadow-sm border border-amber-100">
                <h3 class="mb-4 text-sm font-bold text-gray-800">Resumen de Cotización</h3>
            ...
            <div class="rounded-2xl bg-white p-5 shadow-sm">
                ...
                <flux:select wire:model="estado" label="Estado">
                    @foreach(['borrador','pendiente','confirmado','cancelado'] as $e)
