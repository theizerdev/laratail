<div>
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Completa la información del producto.</p>
        </div>
    ...
    <div class="space-y-6">
        ...
        @if($currentTab === 'info')
            <div class="rounded-2xl bg-white p-6 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <flux:input wire:model="sku" label="SKU" placeholder="Se genera automáticamente si se deja vacío" :error="$errors->first('sku')" />
                </div>
                ...
                <flux:textarea wire:model="descripcion_corta" label="Descripción Corta" placeholder="Breve descripción para listados..." rows="2" :error="$errors->first('descripcion_corta')" />

                <div>
                    <flux:label>Descripción Completa</flux:label>
                    <textarea wire:model="descripcion" rows="6" class="mt-1 block w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500" placeholder="Descripción detallada del producto...">{{ $descripcion }}</textarea>
        ...
        @if($currentTab === 'variants')
            <div class="rounded-2xl bg-white p-6 shadow-sm space-y-4">
                ...
                @if($tiene_variantes)
                    ...
                    <div class="space-y-3">
                        <p class="text-xs text-gray-500">Selecciona los atributos y valores para generar variantes:</p>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($attr->values as $val)
                                        <label class="inline-flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1 text-xs transition
                                            ...
                                            <input type="checkbox"
                                                value="{{ $val->id }}"
                                                wire:model.live="selectedVariantAttributes.{{ $attr->id }}"
                                                class="sr-only"
                                            />
                    ...
                    @if(count($variants) > 0)
                        <div class="space-y-2">
                            <p class="text-xs font-semibold text-gray-700">{{ count($variants) }} variante(s) generada(s):</p>
                            @foreach($variants as $i => $variant)
                                <div class="rounded-xl border border-gray-200 p-3">
                                    ...
                                    <div class="grid grid-cols-2 gap-2 sm:grid-cols-5">
                                        <flux:input wire:model="variants.{{ $i }}.sku" label="SKU" placeholder="SKU" :error="$errors->first('variants.' . $i . '.sku')" />
        ...
        @if($currentTab === 'images')
            <div class="rounded-2xl bg-white p-6 shadow-sm space-y-4">
                ...
                <div>
                    ...
                    <div class="mt-2 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            @if($imagen_principal)
                                <button wire:click="$set('imagen_principal', null)" class="mt-1 text-xs text-red-500 hover:text-red-700">Quitar</button>
                            @endif
                            <div wire:loading wire:target="imagen_principal" class="mt-1 text-xs text-indigo-500">Subiendo...</div>
                ...
                {{-- Additional Images --}}
                <div>
                    <flux:label>Imágenes Adicionales</flux:label>
                    <p class="text-xs text-gray-400 mb-2">Sube múltiples imágenes para la galería del producto.</p>
                    <input type="file" wire:model="imagenes" accept="image/png,image/jpeg,image/webp" multiple class="block w-full text-xs text-gray-500 file:mr-3 file:rounded-lg file:border-0 file:bg-indigo-50 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-indigo-600 hover:file:bg-indigo-100" />
                    <div wire:loading wire:target="imagenes" class="mt-1 text-xs text-indigo-500">Subiendo imágenes...</div>
                    @error('imagenes.*') <p class="mt-1 text-xs text-red-500">{{ $message }}</p> @enderror
                            ...
                            @foreach($imagenes as $idx => $img)
                                @if($img)
                                    <div class="relative h-20 w-20 overflow-hidden rounded-xl border border-gray-200">
                                        <img src="{{ $img->temporaryUrl() }}" alt="Img {{ $idx + 1 }}" class="h-full w-full object-cover" />
                                        <button wire:click="$set('imagenes', @js(array_values(array_filter($imagenes, fn($v, $k) => $k !== $idx, ARRAY_FILTER_USE_BOTH))))" class="absolute -top-1 -right-1 rounded-full bg-red-500 p-0.5 text-white">
        ...
        @if($currentTab === 'seo')
            <div class="rounded-2xl bg-white p-6 shadow-sm space-y-4">
                ...
                <flux:input wire:model="meta_title" label="Meta Title" placeholder="Título para buscadores (max 60 caracteres)" :error="$errors->first('meta_title')" />
                <flux:textarea wire:model="meta_description" label="Meta Description" placeholder="Descripción para buscadores (max 160 caracteres)" rows="3" :error="$errors->first('meta_description')" />
